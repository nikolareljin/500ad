...
- **Food** 🌾: Generated by city agriculture plus fertile/resource-rich hinterlands
- **Wood** 🪵: Gathered from nearby forests and supports industrial growth
- **Stone** 🪨: Sourced from hills/mountains and improves fortification potential
- **Iron** ⛓️: Sourced from mineral-rich terrain and boosts advanced industry output
- **Rare Materials** 💎: Trade-oriented luxury/strategic resources from special regions

### Terrain & Resource Effects
- **Biome-weighted resources**: Plains and river basins favor food, forests favor wood, hills/mountains favor stone/iron, and coastal/special zones can produce rare materials.
- **Movement penalties/bonuses**: Units still use class-based movement rules, but terrain effects now provide a shared base for movement cost adjustments.
- **Combat terrain consistency**: Combat terrain modifiers now use the same terrain-effect definitions used by movement/build systems.
- **Build restrictions by terrain**:
  - `Build Farm` requires fertile/river/coastal land nearby
  - `Irrigate` requires river/coast/fertile lowland access
  - `Build Canal` requires coast or river access
  - `Plant Forest` requires suitable nearby land
- **City production footprint**: Cities benefit from surrounding terrain and nearby strategic deposits rather than only the city tile itself.
...
**Version 1.21.0** - Map rendering improvements: major rivers (Nile, Danube, Euphrates, Tigris, Volga) render wider with distinct colouring; inland lakes (Caspian Sea, Aral Sea, Lake Van, Dead Sea) now appear as water. Introduces the `ScenarioLoader` class and two bundled historical battle scenarios: Battle of Manzikert (1071) and Fall of Constantinople (1453).
